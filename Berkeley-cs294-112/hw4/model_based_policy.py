...
class ModelBasedPolicy(object):
    ...
    def _dynamics_func(self, state, action, reuse):
        ...
        state_delta = utils.build_mlp(x, self._state_dim, 'dynamic_net', self._nn_layers, reuse=reuse)
        next_state_pred = state + utils.unnormalize(state_delta, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
    ...
    def _setup_training(self, state_ph, next_state_ph, next_state_pred):
        ...
        ### PROBLEM 1
        ### YOUR CODE HERE
        real_diff = next_state_ph - state_ph
        pred_diff = next_state_pred - state_ph

        real_diff_norm = utils.normalize(real_diff, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)
        pred_diff_norm = utils.normalize(pred_diff, self._init_dataset.delta_state_mean, self._init_dataset.delta_state_std)

        loss = tf.losses.mean_squared_error(labels=real_diff_norm, predictions=pred_diff_norm)
    ...
    def _setup_action_selection(self, state_ph):
        ...
        ### PROBLEM 2
        ### YOUR CODE HERE
        action_sequences = tf.random_uniform([self._horizon, self._num_random_action_selection, self._action_dim], self._action_space_low, self._action_space_high)
        states = tf.reshape(state_ph, [self._num_random_action_selection, self._state_dim])
        
        costs = tf.zeros([self._num_random_action_selection])
        for i in range(self._horizon):
            next_states = self._dynamics_func(states, action_sequences[i], reuse=True)
            costs += self._cost_fn(states, action_sequences[i], next_states)
            state = next_states

        best_action = action_sequences[tf.argmin(costs)][0]
