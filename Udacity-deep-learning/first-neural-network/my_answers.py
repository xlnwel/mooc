...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        loss = 0.5 * np.mean((y - final_outputs.squeeze())**2)

        dL = final_outputs - y
        dH = np.dot(dL, self.weights_hidden_to_output.T) * hidden_outputs * (1 - hidden_outputs)

        # Weight step (input to hidden)
        delta_weights_i_h += np.dot(X.T, dH)
        # Weight step (hidden to output)
        delta_weights_h_o += np.dot(hidden_outputs.T, dL)
