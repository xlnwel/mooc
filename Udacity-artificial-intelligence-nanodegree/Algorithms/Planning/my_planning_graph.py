...
from lp_utils import decode_state, FluentState
...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        """
        def get_state(level):
            """ get a state object for the current state level
            ...
            for state in self.s_levels[level]:
                (pos if state.is_pos else neg).append(state.symbol)

            return FluentState(pos, neg)

        def check_precondition(kb, action):
            for precond_pos in action.precond_pos:
                if precond_pos not in kb.clauses:
                    return False
            for precond_neg in action.precond_neg:
                if ~precond_neg not in kb.clauses:
                    return False
            return True

        # initial a propositional kb for precondition check
        kb = PropKB()
        kb.tell(get_state(level).sentence())

        # add an action level
        self.a_levels.append(set())
        for action in self.all_actions:
            if check_precondition(kb, action):
                action_node = PgNode_a(action)
                # connect the action node to the previous` literal level
                action_node.parents = {literal for literal in self.s_levels[level]
                                       if literal == action_node.prenodes}
                for literal in action_node.parents:
    ...
    def add_literal_level(self, level):
        ...
        """
        class _CaptureEq:
            """ a class used to capture the equivalent object of obj in a container """
        ...
        def get_equivalent(container, item):
            """ return the equivalent element of item in container"""
            tmp = _CaptureEq(item)
            if tmp in container:
                return tmp.match
            return None
        ...
        # add an literal layer
        self.s_levels.append(set())
        for action_node in self.a_levels[level - 1]:
            for literal_node in action_node.effnodes:
                # get the equivalent node of literal_node in s_levels[level] if there is one
                eq_node = get_equivalent(self.s_levels[level], literal_node)
                # following procedure could be combined into one case if get_equivalent returns
                # item instead of None when there is no equivalent element in the container
                # I leave them here for clarification and avoiding redundant operations
                if eq_node:
                    # literal_node is already in s_levels[level]
                    # disconnect action_node from literal_node
                    action_node.effnodes.remove(literal_node)
                    # connect action_node to the equivalent node in s_levels[level]
                    action_node.effnodes.add(eq_node)
                    action_node.children.add(eq_node)
                    eq_node.parents.add(action_node)
                else:
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len((set(action1.effect_add) & set(action2.effect_rem))
               | (set(action1.effect_rem) & set(action2.effect_rem))):
            return True
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        if len((set(action1.precond_neg) & set(action2.effect_add))
               | (set(action1.precond_pos) & set(action2.effect_rem))
               | (set(action1.effect_rem) & set(action2.precond_pos))
               | (set(action1.effect_add) & set(action2.precond_neg))):
            return True
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        """
        """
        Following implementation is incomplete. 
        Because it only checks whether any of two action's preconditions negate each other,
        and forget the fact that literal mutex could be inconsistent support
        
        action1 = node_a1.action
        action2 = node_a2.action

        if len((set(action1.precond_pos) & set(action2.precond_neg))
               | (set(action1.precond_neg) & set(action2.precond_pos))):
            return True
        
        return False
        """

        if [(s1, s2) for s1 in node_a1.parents for s2 in node_a2.parents if s1.is_mutex(s2)]:
            return True
    ...
    def h_levelsum(self) -> int:
        ...
        :return: int
        """
        def is_in(goal, s_level) -> bool:
            for s_node in s_level:
                if (s_node.is_pos and goal == s_node.symbol) or (not s_node.is_pos and goal == -s_node.symbol):
                    return True
            return False
        ...
        for goal in self.problem.goal:
            for level in range(len(self.s_levels)):
                if is_in(goal, self.s_levels[level]):
                    level_sum += level
                    break
